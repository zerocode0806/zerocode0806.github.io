    ...
    if (!empty($query)) {
        ...
        while ($value = $query->fetch_array()) {
            ...
            $nestedData['aksi'] = '<div class="text-center" width="20%">
                                ...
                                <a href="hapus-mahasiswa.php?id_mahasiswa='.$value['id_mahasiswa'].'" class="btn btn-danger btn-sm" onclick="return confirm("Yakin ingin menghapus data mahasiswa ?")"><i class="fas fa-trash-alt"></i> Hapus</a>
                                </div>';
            $data[] = $nestedData;
            $no++;
        }
